...
\titlespacing*{\section}{0cm}{*1.25}{*1.0}
\titlespacing*{\subsection}{0cm}{*1.0}{*0.35}
\setlist[itemize]{topsep=0.15em, leftmargin=1.5pc}
...
{（数据库测试）学术访问实习}
\begin{itemize}
  \item 为开源数据库测试框架 SQLancer 增加 SurrealDB 支持，包括语法生成器及对应的 PQS、TLP 测试预言模块。
  \item 基于 SQLancer 开发面向跨数据库数据变更捕获工具的自动化测试套件。
\end{itemize}
...
{（分布式数据库）研发实习}
\begin{itemize}
  \item 协助构建基于 Multi-Paxos 协议的地理分布式多节点读写数据库，实现行级多写。
...
{（形式化验证）研发实习}
\begin{itemize}
  \item 使用 ocamllex、ocamlyacc 工具协助团队构建 Lustre V6 编译器项目前端词法与语法分析模块
\end{itemize}
% \end{onehalfspacing}
...
{二等奖（全国第三名）}
\begin{itemize}
  \item 针对低轮 Keccak256 哈希算法的原像攻击，负责算法研究和编程实现
  \item 复现 Asiacrypt 会议的攻击方法，利用 NTL 数学库加速求解过程，并改良特化结构以优化攻击效果
\end{itemize}
% \end{onehalfspacing}
...
\begin{itemize}
    ...
    \item 中南大学 ACM-ICPC 算法竞赛实验室队员，协助管理实验室事务
